...
#define I2C_CTL_STA_SI                                                         \
    (0x28U) /*!< I2C_CTL setting for I2C control bits. It would set STA and SI \
               bits          */
#define I2C_CTL_STA_SI_AA                                                   \
    (0x2CU) /*!< I2C_CTL setting for I2C control bits. It would set STA, SI \
               and AA bits      */
#define I2C_CTL_STO_SI                                                         \
    (0x18U) /*!< I2C_CTL setting for I2C control bits. It would set STO and SI \
               bits          */
#define I2C_CTL_STO_SI_AA                                                   \
    (0x1CU) /*!< I2C_CTL setting for I2C control bits. It would set STO, SI \
               and AA bits      */
#define I2C_CTL_SI \
    (0x08U) /*!< I2C_CTL setting for I2C control bits. It would set SI bit */
#define I2C_CTL_SI_AA                                                         \
    (0x0CU) /*!< I2C_CTL setting for I2C control bits. It would set SI and AA \
               bits           */
#define I2C_CTL_STA \
    (0x20U) /*!< I2C_CTL setting for I2C control bits. It would set STA bit */
#define I2C_CTL_STO \
    (0x10U) /*!< I2C_CTL setting for I2C control bits. It would set STO bit */
#define I2C_CTL_AA \
    (0x04U) /*!< I2C_CTL setting for I2C control bits. It would set AA bit */
...
/*---------------------------------------------------------------------------------------------------------*/
#if 0 /* bccho, 2025-09-22 */
#define I2C_TIMEOUT \
    SystemCoreClock /*!< I2C time-out counter (1 second time-out) */
#else
#define I2C_TIMEOUT (SystemCoreClock / 10)
#endif
 ...
 *    @return       None
 *
 *    @details      Set I2C_CTL register to control I2C bus conditions of START,
 * STOP, SI, ACK.
 */
...
#define I2C_START(i2c) \
    ((i2c)->CTL0 = ((i2c)->CTL0 | I2C_CTL0_SI_Msk) | I2C_CTL0_STA_Msk)
 ...
 *    @return       None
 *
 *    @details      When a new status is presented of I2C bus, the SI flag will
 * be set in I2C_CTL register.
 */
 ...
 *    @return       None
 *
 *    @details      When write a data to I2C_DAT register, the I2C controller
 * will shift it to I2C bus.
 */
 ...
 *
 *    @details      When I2C bus occurs time-out event, the time-out flag will
 * be set.
 */
#define I2C_GET_TIMEOUT_FLAG(i2c) \
    (((i2c)->TOCTL & I2C_TOCTL_TOIF_Msk) == I2C_TOCTL_TOIF_Msk ? 1u : 0u)
...
#define I2C_GET_WAKEUP_FLAG(i2c) \
    (((i2c)->WKSTS & I2C_WKSTS_WKIF_Msk) == I2C_WKSTS_WKIF_Msk ? 1u : 0u)
 ...
 *    @retval       1       The ACK bit cycle of address match frame is done in
 * power-down
 *
 *    @details      I2C bus occurs wake-up event and address frame ACK is done,
 * this flag will be set.
 *
 *    \hideinitializer
 */
#define I2C_GET_WAKEUP_DONE(i2c)                                            \
    (((i2c)->WKSTS & I2C_WKSTS_WKAKDONE_Msk) == I2C_WKSTS_WKAKDONE_Msk ? 1u \
                                                                       : 0u)
 ...
 *    @param[in]    i2c     Specify I2C port
 *
 *    @retval       0       Write command be record on the address match wakeup
 * frame
 *    @retval       1       Read command be record on the address match wakeup
 * frame.
 *
 *    @details      I2C bus occurs wake-up event and address frame is received,
 * this bit will record read/write status.
 *
 *    \hideinitializer
 */
#define I2C_GET_WAKEUP_WR_STATUS(i2c) \
    (((i2c)->WKSTS & I2C_WKSTS_WRSTSWK_Msk) == I2C_WKSTS_WRSTSWK_Msk ? 1u : 0u)
 ...
 * @return     None
 *
 * @details    Device Mode(BMHEN=0): If ALERTEN(I2C_BUSCTL[4]) is set, the Alert
 * pin will pull lo, and reply ACK when get ARP from host Host   Mode(BMHEN=1):
 * If ALERTEN(I2C_BUSCTL[4]) is set, the Alert pin is supported to receive alert
 * state(Lo trigger)
 *
 */
 ...
 * @return     None
 *
 * @details    The 9th bit can response the ACK or NACK according the received
 * data by user. When the byte is received, SCLK line stretching to low between
 ...
 * @return     None
 *
 * @details    This function is used to enable the condition of REAEAT START can
 ...
 * @return     None
 *
 * @details    This function is used to disable the condition of Repeat START
 ...
 * @return     None.
 * @details    Set RXPDMAEN bit of I2C_CTL1 register to enable RX PDMA transfer
 * function.
 */
 ...
 * @param[in]  i2c The pointer of the specified I2C module.
 * @return     None.
 * @details    Set TXPDMAEN bit of I2C_CTL1 register to enable TX PDMA transfer
 * function.
 */
...
#define I2C_DISABLE_RST_PDMA(i2c) ((i2c)->CTL1 |= I2C_CTL1_PDMARST_Msk)
    ...
    void I2C_Trigger(I2C_T *i2c, uint8_t u8Start, uint8_t u8Stop, uint8_t u8Si,
                     uint8_t u8Ack);
    ...
    void I2C_SetSlaveAddr(I2C_T *i2c, uint8_t u8SlaveNo, uint8_t u8SlaveAddr,
                          uint8_t u8GCMode);
    void I2C_SetSlaveAddrMask(I2C_T *i2c, uint8_t u8SlaveNo,
    ...
    uint32_t I2C_WriteMultiBytes(I2C_T *i2c, uint8_t u8SlaveAddr,
                                 uint8_t au8Data[], uint32_t u32wLen);
    uint8_t I2C_WriteByteOneReg(I2C_T *i2c, uint8_t u8SlaveAddr,
                                uint8_t u8DataAddr, uint8_t u8Data);
    uint32_t I2C_WriteMultiBytesOneReg(I2C_T *i2c, uint8_t u8SlaveAddr,
                                       uint8_t u8DataAddr, uint8_t au8Data[],
                                       uint32_t u32wLen);
    uint8_t I2C_WriteByteTwoRegs(I2C_T *i2c, uint8_t u8SlaveAddr,
                                 uint16_t u16DataAddr, uint8_t u8Data);
    uint32_t I2C_WriteMultiBytesTwoRegs(I2C_T *i2c, uint8_t u8SlaveAddr,
                                        uint16_t u16DataAddr, uint8_t au8Data[],
                                        uint32_t u32wLen);
    uint8_t I2C_ReadByte(I2C_T *i2c, uint8_t u8SlaveAddr);
    uint32_t I2C_ReadMultiBytes(I2C_T *i2c, uint8_t u8SlaveAddr,
                                uint8_t au8Rdata[], uint32_t u32rLen);
    uint8_t I2C_ReadByteOneReg(I2C_T *i2c, uint8_t u8SlaveAddr,
                               uint8_t u8DataAddr);
    uint32_t I2C_ReadMultiBytesOneReg(I2C_T *i2c, uint8_t u8SlaveAddr,
                                      uint8_t u8DataAddr, uint8_t au8Rdata[],
                                      uint32_t u32rLen);
    uint8_t I2C_ReadByteTwoRegs(I2C_T *i2c, uint8_t u8SlaveAddr,
                                uint16_t u16DataAddr);
    uint32_t I2C_ReadMultiBytesTwoRegs(I2C_T *i2c, uint8_t u8SlaveAddr,
                                       uint16_t u16DataAddr, uint8_t au8Rdata[],
                                       uint32_t u32rLen);
