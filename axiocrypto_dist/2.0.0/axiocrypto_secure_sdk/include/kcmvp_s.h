...
void set_stack_top(const char *);
void set_stack_ref(void);
int32_t get_stack_max_depth(void);
const char * get_stack_max_location(void);
void report_stack_usage(const char *func);

#define AXIOCRYPTO_SET_STACK_REF set_stack_ref()
#define AXIOCRYPTO_CHECK_STACK set_stack_top(__func__)
#define AXIOCRYPTO_REPORT_STACK report_stack_usage(__func__);
#define DEBUG_SKIP_SELFTEST 1
#else
#define AXIOCRYPTO_SET_STACK_REF
#define AXIOCRYPTO_CHECK_STACK
#define AXIOCRYPTO_REPORT_STACK
#endif
...
CRYPTO_STATUS nsc_axiocrypto_get_module_status(uint32_t magic);
#if defined(AXIOCRYPTO_FAULT_INDUCTION)
CRYPTO_STATUS nsc_set_error(uint32_t magic, void *v[2]);
CRYPTO_STATUS nsc_show_keystorage(uint32_t magic);
CRYPTO_STATUS nsc_drbg_set_context(uint32_t magic, void *v[3]);
CRYPTO_STATUS nsc_drbg_init(uint32_t magic, void *v[6]);
CRYPTO_STATUS nsc_drbg_update(uint32_t magic, void *v[4]);
CRYPTO_STATUS nsc_drbg_final(uint32_t magic, void *v[9]);
CRYPTO_STATUS nsc_drbg(uint32_t magic, void *v[15]);
#endif
