...
uint16_t crc16_ccitt_get(const void *buf, uint32_t size)
{
    ...
    while (size--)
    {
        crc = (crc << 8) ^ crc16_ccitt_tab[((crc >> 8) ^ (*p++)) & 0xFF];
    }
...
int crc16_compare(void *buff, uint32_t size)
{
    ...
    trialcrc = crc16_get(cp, size);
    cp[size] = (trialcrc & 0x00ff);
    cp[size + 1] = ((trialcrc >> 8) & 0x00ff);

    trialcrc = crc16_get_nc(cp, size + 2);
    if (trialcrc == 0xf0b8)
        return 0;
    else
        return 1;
...
uint16_t extflash_crc16_tab_get(uint32_t addr, uint32_t len)
{
    ...
    for (int i = 0; i < len; i++)
    {
        ...
        wait_SPI_IS_BUSY(SPI0);
        uint8_t buf = (uint8_t)SPI_READ_RX(SPI0);
    ...
    OS_ENTER_CRITICAL();
    mx25r4035f_request_cs();

    mx25r4035f_spi(FLASH_CMD_FASTREAD, 0);
    SendFlashAddr(addr, SIO, FALSE);
